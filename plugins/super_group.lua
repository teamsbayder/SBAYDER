...
local function xsbayder(msg, matches)
if msg.to.type ~= 'pv' then
if matches[1] == "تفعيل" and not matches[2] then local numusersax = (tonumber(redis:get(sbayder..':addnumberusers')) or 0) return modadd(msg,numusersax)  end
if matches[1] == "تعطيل" and not matches[2] then return modrem(msg) end end
if msg.to.type ~= 'pv' and redis:get(sbayder..'group:add'..msg.to.id) then 
if matches[1] == "ايدي" then
if not matches[2] and not msg.reply_id then
if msg.from.username then userxn = "@"..msg.from.username else userxn = "لا يوجد !" end
msgs = tonumber(redis:get(sbayder..'msgs:'..msg.from.id..':'..msg.to.id) or 1)
if redis:get(sbayder..'lock_id'..msg.to.id) then
...
redis:setex(sbayder..'waiting_link'..msg.from.id,300,true)
return "🙋🏼‍♂️*¦* أهلا عزيزي \n🔖¦ رجائا ارسل الرابط الآن \n🔃"
end
if matches[1] == "الرابط" then
if not redis:get(sbayder..'linkgp'..msg.to.id) then return "📡*¦* اوه 🙀 لا يوجد هنا رابط\n🔖¦ *رجائا اكتب [ضع رابط]* \n🔃" end
return sendMsg(msg.to.id,msg.id_,"`🔖¦رابـط الـمـجـمـوعه 💯\n🌿¦"..redis:get(sbayder..'group:name'..msg.to.id).." :\n\n`["..redis:get(sbayder..'linkgp'..msg.to.id).."]\n",'md')
end
...
redis:setex(sbayder..'rulse:witting'..msg.from.id,300,true)
return '📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل القوانين  للمجموعه 🍃'
end
if matches[1] == "القوانين" then
if not redis:get(sbayder..'rulse:msg'..msg.to.id) then return "📡*¦* مرحبأ عزيري 👋🏻 القوانين كلاتي 👇🏻\n🔖¦ ممنوع نشر الروابط \n🔖¦ ممنوع التكلم او نشر صور اباحيه \n🔖¦ ممنوع  اعاده توجيه\n🔖¦ ممنوع التكلم بلطائفه \n🔖¦ الرجاء احترام المدراء والادمنيه 😅\n"
else return "*🔖¦القوانين :*\n"..redis:get(sbayder..'rulse:msg'..msg.to.id) end 
end
...
redis:set(sbayder..'flood'..msg.to.id,matches[2]) 
...
redis:del(sbayder..'admins:'..msg.to.id)
return "🙋🏼‍♂️*¦* أهلا عزيزي  \n📛¦ تم حذف "..#list.." ادمن من البوت ✓"
end
if matches[2] == "قائمه المنع" and is_owner(msg) then
if #redis:smembers(sbayder..'klmamn3'..msg.to.id) == 0 then return "📡*¦* عذرا لا توجد كلمات ممنوعه ليتم حذفها ✓" end
redis:del(sbayder..'klmamn3'..msg.to.id)
return "🙋🏼‍♂️*¦* أهلا عزيزي  \n🔖¦ تم حذف الكلمات الممنوعه بنجاح ✓"
end
if matches[2] == "القوانين" and is_owner(msg) then
if not redis:get(sbayder..'rulse:msg'..msg.to.id) then return "📡*¦* اوه ☢ هنالك خطأ 🚸\n📛¦ عذرا لا يوجد قوانين ليتم مسحه ✓" end
redis:del(sbayder..'rulse:msg'..msg.to.id)
return "🙋🏼‍♂️*¦* أهلا عزيزي  \n📛¦ تم حذف القوانين بنجاح ✓"
end
if matches[2] == "الترحيب" and is_owner(msg)  then
if not redis:get(sbayder..'welcome:msg'..msg.to.id) then return "📡*¦* اوه ☢ هنالك خطأ 🚸\n📛¦ عذرا لا يوجد ترحيب ليتم مسحه ✓" end
redis:del(sbayder..'welcome:msg'..msg.to.id)
return "🙋🏼‍♂️*¦* أهلا عزيزي  \n📛¦ تم حذف الترحيب بنجاح ✓"
end
if matches[2] == "المدراء" and is_sudo(msg) then
if #redis:smembers(sbayder..'owners:'..msg.to.id) ==0 then return "📡*¦* اوبس ☢ هنالك خطأ 🚸\n📛¦ عذرا لا يوجد مدراء ليتم مسحهم ✓" end
redis:del(sbayder..'owners:'..msg.to.id)
return "🙋🏼‍♂️*¦* أهلا عزيزي  \n📛¦ تم حذف المدراء بنجاح ✓"
end
if matches[2] == 'المحظورين' and is_owner(msg) then
if #redis:smembers(sbayder..'banned:'..msg.to.id) ==0 then return "*📌¦ لا يوجد مستخدمين محظورين  *" end
redis:del(sbayder..'banned:'..msg.to.id)
return "📡*¦* تم مسح المحظورين في المجموعه \n✓"
end
if matches[2] == 'المكتومين' and is_owner(msg)  then
if #redis:smembers(sbayder..'is_silent_users:'..msg.to.id) ==0 then return "📡*¦* لا يوجد مستخدمين مكتومين في المجموعه " end
redis:del(sbayder..'is_silent_users:'..msg.to.id)
return "⚙️*¦* تم مسح قائمه الكتم \n✓"
end
if matches[2] == 'المميزين' and is_owner(msg)  then
if #redis:smembers(sbayder..'whitelist:'..msg.to.id) ==0 then return "*⚙️*¦ لا يوجد مستخدمين مميزين في المجموعه " end
redis:del(sbayder..'whitelist:'..msg.to.id)
return "⚙️*¦* تم مسح قائمه المميزين \n✓"
end
if matches[2] == 'الرابط' and is_owner(msg)  then
if not redis:get(sbayder..'linkgp'..msg.to.id) then return "*⚙️*¦ لا يوجد رابط مضاف اصلا " end
redis:del(sbayder..'linkgp'..msg.to.id)
return "⚙️*¦* تم مسح رابط المجموعه \n✓"
end
end --end del 
if matches[1] == "ضع اسم" and is_mod(msg) then
redis:setex(sbayder..'name:witting'..msg.from.id,300,true)
...
else redis:setex(sbayder..'photo:group'..msg.to.id..msg.from.id,300,true)
return '📭¦ حسننا عزيزي 🍁\n🌄 ¦ الان قم بارسال الصوره\n🛠' end end
if matches[1] == "ضع وصف" and is_mod(msg) then redis:setex(sbayder..'about:witting'..msg.from.id,300,true) return "📭¦ حسننا عزيزي  ✋🏿\n🗯¦ الان ارسل الوصف  للمجموعه\n🛠" end
...
redis:setex(sbayder..'ExpireDate:'..msg.to.id, extime, true)
if not redis:get(sbayder..'CheckExpire::'..msg.to.id) then redis:set(sbayder..'CheckExpire::'..msg.to.id,true) end
sendMsg(msg.to.id,msg.id_,'💂🏻‍♀️¦ تم شحن الاشتراك الى `'..matches[2]..'` يوم   ... 👍🏿','md')
sendMsg(SUDO_ID, 0,'💂🏻‍♀️¦ تم شحن الاشتراك الى `'..matches[2]..'` يوم   ... 👍🏿\n🕵🏼️‍♀️¦ في مجموعه  » »  '..redis:get(sbayder..'group:name'..msg.to.id),'md')
else
...
return '🗯*¦* عدد رسـآئلگ هہيهہ‏‏» (*'..tonumber(redis:get(sbayder..'msgs:'..msg.from.id..':'..msg.to.id) or 0)..'*) رساله \n' end
 if matches[1]== 'جهاتي' then
 return '🧟‍♂*¦*  عدد جهہآتگ آلمـضـآفهہ‏‏ » 【'..(tonumber(redis:get(sbayder..':addusers_group:'..msg.to.id..':'..msg.from.id)) or 0)..'】 . \n🐾'
end
...
local msgs = tonumber(redis:get(sbayder..'msgs:'..msg.from.id..':'..msg.to.id) or 0)
sendMsg(msg.to.id, msg.id_,'*👨🏽‍🔧¦ اهـلا بـك عزيزي » \n\n📜¦ الاسم » * ['..check_name(namecut(msg.from.first_name))..']\n*🎟¦ المعرف:* ['..username..']\n*🏷¦ الايدي » * 【 `'..msg.from.id..'` 】\n📨*¦* رسائلك » 【*'..msgs..'*】رسـآلهہ‏‏ \n🧟‍♂*¦*  عدد جهہآتگ » 【'..(tonumber(redis:get(sbayder..':addusers_group:'..msg.to.id..':'..msg.from.id)) or 0)..'】جهہ‏‏\n⭐️*¦* تفاعلك » '..get_ttl(msgs)..'\n*🎗¦ رتبتك » * '..get_rank(msg)..'\n*🎫¦ ايدي المجموعه » * ( `'..msg.to.id..'` ) \n\n*🏌🏻¦ مـطـور البوت * » '..SUDO_USER..'\n👨🏽‍🔧','md') end
...
if matches[2] == "الرد بالرد" then redis:set(sbayder..":msg_id"..msg.to.id,true) return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تفعيل الرد بالرد \n✓" end
...
if redis:get(sbayder..'lock_id'..msg.to.id) then return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* امر الايدي شغال بالفعل\n✓" else redis:set(sbayder..'lock_id'..msg.to.id,true)  return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تفعيل امر الايدي \n✓" end end
if matches[2] == "الترحيب" then 
if redis:get(sbayder..'welcome:get'..msg.to.id) then return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تفعيل الترحيب مفعل مسبقاً\n✓" else redis:set(sbayder..'welcome:get'..msg.to.id,true)  return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تفعيل الترحيب \n✓" end end
if matches[2] == "التحذير" then 
if redis:get(sbayder..'lock_woring'..msg.to.id) then return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تفعيل التحذير مفعل مسبقاً \n✓" else redis:set(sbayder..'lock_woring'..msg.to.id,true) return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تفعيل التحذير \n✓" end end end
if matches[1] == "تعطيل" and is_mod(msg) then
if matches[2] == "الرد بالرد" then redis:del(sbayder..":msg_id"..msg.to.id) return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تعطيل الرد بالرد \n✓" end
if matches[2] == "الردود" then return lock_replay(msg) end
if matches[2] == "الاذاعه" and is_sudo(msg) then return lock_brod(msg) end
if matches[2] == "الايدي" then
if not redis:get(sbayder..'lock_id'..msg.to.id) then return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* الايدي بالتأكيد معطل\n✓" else redis:del(sbayder..'lock_id'..msg.to.id) return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تعطيل امر الايدي\n✓" end end
if matches[2] == "الترحيب" then
if not redis:get(sbayder..'welcome:get'..msg.to.id) then return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* الترحيب بالتأكيد معطل\n✓" else redis:del(sbayder..'welcome:get'..msg.to.id) return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تعطيل الترحيب \n✓" end end
if matches[2] == "التحذير" then
if not redis:get(sbayder..'lock_woring'..msg.to.id) then return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* التحذير بالتأكيد معطل\n✓" else redis:del(sbayder..'lock_woring'..msg.to.id) return "🙋🏼‍♂️*¦* أهلا عزيزي \n📡*¦* تم تعطيل التحذير \n✓" end end end
if matches[1] == "ضع الترحيب" and is_mod(msg) then redis:set(sbayder..'welcom:witting'..msg.from.id,true) return "📭¦ حسننا عزيزي  ✋🏿\n🗯¦ ارسل كليشه الترحيب الان\n🛠" end
if matches[1] == "الترحيب"  and is_mod(msg) then
if redis:get(sbayder..'welcome:msg'..msg.to.id)  then
return redis:get(sbayder..'welcome:msg'..msg.to.id)
...
if #redis:smembers(sbayder..':SUDO_BOT:') == 0 then  return "⚙️*¦* عذرا لا يوجد مطورين في البوت  ✖️" end
sendMsg(msg.to.id,msg.id_,"📛*¦* تم مسح `"..redis:scard(sbayder..':SUDO_BOT:').."` من المطورين ☔️\n✓","md")
redis:del(sbayder..':SUDO_BOT:') return false end			
if matches[1] == 'مسح' and matches[2] == "قائمه العام" and we_sudo(msg)  then
if #redis:smembers(sbayder..'gban_users')==0 then return "*⚙️¦ لا يوجد مستخدمين محظورين عام في المجموعه *" end
redis:del(sbayder..'gban_users') return "⚙️*¦* تم مسح قائمه العام\n✓" end 
if we_sudo(msg) then
...
redis:setex(sbayder..'welcom_ph:witting'..msg.from.id,300,true) return'📭¦ حسننا عزيزي 🍁\n🌄 ¦ الان قم بارسال الصوره للترحيب \n🛠' end
...
return sendPhoto(msg.to.id,msg.id_,0,1,nil,redis:get(sbayder..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(sbayder..'bot:name')..[[ 🎖
💰¦ آختصـآصـي حمـآيهہ‏‏ آلمـجمـوعآت
📛¦ مـن آلسـبآم وآلتوجيهہ‏‏ وآلتگرآر وآلخ...
⚖️¦ مـعرف آلمـطـور  » ]]..SUDO_USER:gsub([[\_]],'_')..[[ 🌿
👨🏽‍🔧]],dl_cb,nil) end
if matches[1] == "ضع كليشه المطور" then 
redis:setex(sbayder..'text_sudo:witting'..msg.from.id,1200,true) return'📭¦ حسننا عزيزي 🍁\n💬¦ الان قم بارسال الكليشه \n🛠' end
if matches[1] == "ضع شرط التفعيل" and matches[2] and string.match(matches[2], '^%d+$') then 
redis:set(sbayder..':addnumberusers',matches[2]) return'💱*¦* تم وضـع شـرط آلتفعيل آلبوت آذآ گآنت آلمـجمـوعهہ‏‏ آگثر مـن*【'..matches[2]..'】* عضـو  🍁\n' end
if matches[1] == "شرط التفعيل" then return'🚸*¦* شـرط آلتفعيل آلبوت آذآ گآنت آلمـجمـوعهہ‏‏ آگثر مـن*【'..redis:get(sbayder..':addnumberusers')..'】* عضـو  🍁\n' end end
if is_sudo(msg) then
...
if redis:sismember(sbayder..'group:ids',matches[2]) then
local name_gp = redis:get(sbayder..'group:name'..matches[2])
...
sendMsg(msg.to.id,msg.id_,(redis:get(sbayder..":TEXT_SUDO") or '🗃¦ لا توجد كليشه المطور .\n📰¦ يمكنك اضافه كليشه من خلال الامر\n       " `ضع كليشه المطور` " \n📡'),'md') end
if (matches[1] == "اذاعه عام بالتوجيه" or matches[1] == "اذاعه عام بالتوجيه 📣") and is_sudo(msg) then		
if not we_sudo(msg) and not redis:get(sbayder..'lock_brod') then  return "هذا الاوامر للمطور الاساسي فقط "  end
redis:setex(sbayder..'fwd:'..msg.from.id,300, true) return "📭¦ حسننا الان ارسل التوجيه للاذاعه \n🔛" end
if (matches[1] == "اذاعه عام" or matches[1] == "اذاعه عام 📢") and is_sudo(msg) then		
if not we_sudo(msg) and not redis:get(sbayder..'lock_brod') then  return "هذا الاوامر للمطور الاساسي فقط " end
redis:setex(sbayder..'fwd:all'..msg.from.id,300, true) return "📭¦ حسننا الان ارسل الكليشه للاذاعه عام \n🔛" end
if (matches[1] == "اذاعه خاص" or matches[1] == "اذاعه خاص 👤") and is_sudo(msg) then		
if not we_sudo(msg) and not redis:get(sbayder..'lock_brod') then  return "هذا الاوامر للمطور الاساسي فقط "  end
redis:setex(sbayder..'fwd:pv'..msg.from.id,300, true) return "📭¦ حسننا الان ارسل الكليشه للاذاعه خاص \n🔛"	 end
if (matches[1] == "اذاعه" or matches[1] == "اذاعه 🗣") and is_sudo(msg) then		
if not we_sudo(msg) and not redis:get(sbayder..'lock_brod') then  return "هذا الاوامر للمطور الاساسي فقط "  end
redis:setex(sbayder..'fwd:groups'..msg.from.id,300, true) return "📭¦ حسننا الان ارسل الكليشه للاذاعه للمجموعات \n🔛" end
...
redis:del(sbayder..'waiting_link'..msg.from.id) redis:del(sbayder..'welcom:witting'..msg.from.id) redis:del(sbayder..'rulse:witting'..msg.from.id)redis:del(sbayder..'rulse:witting'..msg.from.id) redis:del(sbayder..'name:witting'..msg.from.id) redis:del(sbayder..'about:witting'..msg.from.id) redis:del(sbayder..'fwd:all'..msg.from.id) redis:del(sbayder..'fwd:pv'..msg.from.id) redis:del(sbayder..'fwd:groups'..msg.from.id) redis:del(sbayder..'namebot:witting'..msg.from.id) redis:del(sbayder..'addrd_all:'..msg.from.id) redis:del(sbayder..'delrd:'..msg.from.id) redis:del(sbayder..'addrd:'..msg.from.id) redis:del(sbayder..'delrdall:'..msg.from.id) redis:del(sbayder..'text_sudo:witting'..msg.from.id)  return '📬*¦* تم آلغآء آلآمـر بنجآح \n🌿'  end
----------------- استقبال الرسائل ---------------
if msg.text and msg.text ~= "الغاء الامر ✖️" and msg.text ~= "الغاء الامر"  and msg.text ~= "الغاء" then
if (msg.text:match("^([https?://w]*.?telegram.me/joinchat/%S+)$") or msg.text:match("^([https?://w]*.?t.me/joinchat/%S+)$")) and redis:get(sbayder..'waiting_link'..msg.from.id) then  -- استقبال الرابط
redis:set(sbayder..'linkgp'..msg.to.id,msg.text)
return "💌*¦* شكرأ لك 😻\n📦*¦* تم حفظ الرابط بنجاح \n✓" end
if redis:get(sbayder..'text_sudo:witting'..msg.from.id) then -- استقبال كليشه المطور
redis:del(sbayder..'text_sudo:witting'..msg.from.id) redis:set(sbayder..':TEXT_SUDO','['..msg.text..']')
return "📜*¦* تم وضع الكليشه بنجاح كلاتي 👋🏻\n\n*{*  ["..msg.text.."]  *}*\n✓"  end
if redis:get(sbayder..'welcom:witting'..msg.from.id) then -- استقبال كليشه الترحيب
redis:del(sbayder..'welcom:witting'..msg.from.id) redis:set(sbayder..'welcome:msg'..msg.to.id,msg.text)
return "📜*¦* تم وضع الترحيب بنجاح كلاتي 👋🏻\n*["..msg.text.."]*\n\n🔖¦ ملاحظه تستطيع\n🗒¦ اضهار القوانين بواسطه  ➣ *{rules}*  \n📰¦  اضهار الاسم بواسطه ➣ *{name}*\n🏷¦ اضهار المعرف بواسطه ➣ *{username}*" end
if redis:get(sbayder..'rulse:witting'..msg.from.id) then --- استقبال القوانين
redis:del(sbayder..'rulse:witting'..msg.from.id) redis:set(sbayder..'rulse:msg'..msg.to.id,check_markdown(msg.text)) return '📜*¦* مرحبآ عزيزي\n📦¦ تم حفظ القوانين بنجاح ✓\n🔖¦ اكتب [[ القوانين ]] لعرضها \n💬✓' end
if redis:get(sbayder..'name:witting'..msg.from.id) then --- استقبال الاسم
redis:del(sbayder..'name:witting'..msg.from.id) tdcli_function ({ID = "ChangeChatTitle",chat_id_ = msg.to.id,title_ = msg.text},dl_cb,nil) end
if redis:get(sbayder..'about:witting'..msg.from.id) then --- استقبال الوصف
redis:del(sbayder..'about:witting'..msg.from.id) tdcli_function ({ID = "ChangeChannelAbout",channel_id_ = getChatId(msg.to.id).ID,about_ = msg.text}, dl_cb, nil) return "📜*¦* تم وضع الوصف بنجاح\n✓" end
if redis:get(sbayder..'fwd:all'..msg.from.id) then ---- استقبال رساله الاذاعه عام
redis:del(sbayder..'fwd:all'..msg.from.id)
local pv = redis:smembers(sbayder..'users')  
local groups = redis:smembers(sbayder..'group:ids')
