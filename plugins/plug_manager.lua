...
local update = redis:get(sbayder..":VERSION")
...
redis:srem(sbayder..':PLUGINS_FILE:',matches[2])
sendMsg(msg.to.id,msg.id_,'تم حذف الملف \n↝ ['..matches[2]..']\n','md')
reload_plugins()
end end 

if matches[1] == 'اصدار السورس' or matches[1] == 'الاصدار' then
return '👨🏾‍🔧¦ اصدار سورس سبايدر : `v'..update..'` \n📡'
end
if (matches[1] == 'تحديث السورس' or matches[1] == 'تحديث السورس ™') and is_sudo(msg) then
if not we_sudo(msg)  then return "📛*￤* هذا الاوامر للمطور الاساسي فقط 🛠" end
return"👨🏾‍🔧*¦* انتهى دعم هذا السورس بالتحديثات ترقبو السورس الجديد وبالتيجي الجديد في قناة سبايدر @THSBAYDER . \n📦"
end
